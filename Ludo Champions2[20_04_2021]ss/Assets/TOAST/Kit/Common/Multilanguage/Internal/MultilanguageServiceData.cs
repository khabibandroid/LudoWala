...
namespace Toast.Kit.Common.Multilanguage.Internal
{
    ...
    public class MultilanguageServiceData
    {
        ...
        public MultilanguageResultCode Initialize(MultilanguageXml languageXml)
        {
            ...
            foreach (var data in languageXml.stringList.list)
            {
                ...
                foreach (var language in languages.list)
                {
                    ...
                    {
                        ToastKitLogger.Warn(string.Format("Already have a string key. (key: {0}, language: {1})", key, languageCode), serviceName, GetType());
                    }
                    else
                    {
        ...
        public MultilanguageResultCode SelectLanguage(string languageCode)
        {
            ...
            {
                ToastKitLogger.Error("Language code not found.", serviceName, GetType());
        ...
        public string GetString(string stringKey)
        {
            ...
            {
                ToastKitLogger.Error("Language file is not loaded.", serviceName, GetType());
                return stringKey;
            }

            if (selectLanguageStrings.ContainsKey(stringKey) == false)
            {
                ToastKitLogger.Error(string.Format("String key not found. (key= {0})", stringKey), serviceName, GetType());
                return stringKey;
            }
            ...
            {
                ToastKitLogger.Error(string.Format("String value is null or empty. (key= {0})", stringKey), serviceName, GetType());
                return stringKey;
            }
