...
namespace Toast.Kit.Common.Compress.Internal
{
    internal static class CompressImplementation
    {
        ...
        public static class UnityPackage
        {
            public static CompressResultCode Unpack(string packagePath, string tempPath, string resultPath)
            {
                ...
                string tempFullPath = ToastKitPathUtil.Combine(tempPath, filename);
            ...
            private static bool RemapPackageToAsset(string originPath, string outputPath)
            {
                ...
                try
                {
                    ...
                    {
                        var remapPath = File.ReadAllLines(ToastKitPathUtil.Combine(directoryInfo.FullName, "pathname")).First();
                        remapPath = remapPath.Replace('/', Path.DirectorySeparatorChar);

                        var assetFilePath = ToastKitPathUtil.Combine(directoryInfo.FullName, "asset");
                        var metaFilePath = ToastKitPathUtil.Combine(directoryInfo.FullName, "asset.meta");

                        ToastKitFileUtil.MoveFile(assetFilePath, ToastKitPathUtil.Combine(outputPath, remapPath));
                        ToastKitFileUtil.MoveFile(metaFilePath, ToastKitPathUtil.Combine(outputPath, string.Format("{0}.meta", remapPath)));
                    }
                }
                catch (Exception e)
                {
