...
{
    internal class ToastKitServiceDownloader
    {
            ...
            Action downloadAction = () =>
            {
                foreach (var package in service.packageList)
                {
                    downloadQueue.Enqueue(new PackageInstallInfo
                    {
                        ...
                        packageIntallPath = package.installPath
                    });
                }
        ...
        {
            string url = ToastKitPathUtil.Combine(ToastKitManager.Instance.CdnUri, serviceName, ManagerPaths.SERVICE_FILE_NAME);
        ...
        private IEnumerator DownloadPackages(Action<ManagerError, List<PackageInstallInfo>> callback)
        {
            ...
            while (downloadQueue.Count > 0)
            {
                ...
                string downloadUrl = ToastKitPathUtil.UrlCombine(ToastKitManager.Instance.CdnUri, downloadInfo.serviceName, downloadInfo.packagePath);
                string localFilePath = ManagerPaths.GetCachingPath(downloadInfo.serviceName, downloadInfo.serviceVersion, downloadInfo.packagePath);
