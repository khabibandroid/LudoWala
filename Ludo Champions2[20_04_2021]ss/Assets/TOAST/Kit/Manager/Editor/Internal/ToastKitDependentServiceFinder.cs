...
{
    internal static class ToastKitDependentServiceFinder
    {
            ...
            EditorCoroutine.Start(RefreshDepedencyInfo(installedServices, findService.title,
                (dependecies, error) =>
                {
                    ...
                    else
                    {
                        ...
                        callback(new ManagerError(ManagerErrorCode.UNINSTALL, string.Format(ManagerInfos.GetString(ManagerStrings.ERROR_MESSAGE_DEPENDENCY_SERVICE_REMOVE_FAILED), builder.ToString())));
                    }
                }));
        }
