...
{
    internal class ToastKitServiceDetail : UiView
    {
        ...
        public ToastKitServiceDetail(ToastKitManagerWindow window) : base(window)
        {
        }
        ...
        public void LoadService(string name)
        {
            ...
            ToastKitManager.Instance.LoadServiceInfo(
                name,
                (info, error) =>
                {
                    service = info;
                    ...
                    if (service != null)
                    {
                        installedVersion = ToastKitManager.Instance.Install.GetInstallVersion(service.title);
                        ...
                        ToastKitManager.Instance.DownloadServiceImage(service);
                    }
                        ...
                        case ServiceStatus.PUBLISH:
                            {
                                ...
                                {
                                    EditorGUI.BeginDisabledGroup(installableUnityVersion == false || ToastKitManager.ServiceInstaller.IsProcessing == true);
                                    {
                                        if (ManagerUi.Button(ManagerStrings.SERVICE_INSTALL, GUILayout.ExpandWidth(false)) == true)
                                        {
                                            ToastKitManager.ServiceInstaller.Install(service, OnServiceInstallCompleted);
                                        }
                                    }
                                    EditorGUI.EndDisabledGroup();
                                }
                                else
                                {
                                    ...
                                    if (ManagerUi.Button(ManagerStrings.SERVICE_UNINSTALL, GUILayout.ExpandWidth(false)) == true)
                                    {
                                        ...
                                        {
                                            builder.AppendFormat("\n- {0}", package.installPath);
                                        }

                                        ToastKitManager.IsLock = true;
                                        ToastKitDependentServiceFinder.Process(service, ToastKitManager.Instance.Install.installs, (error) =>
                                        {
                                            ToastKitManager.IsLock = false;

                                            if (error != null)
                                            {
                                                ToastKitManager.Instance.CheckServiceInfoVersion(error);
                                                return;
                                            }

                                            if (ManagerUi.TryDialog(ManagerStrings.SERVICE_REMOVE_TRY_TITLE, builder.ToString(), false) == true)
                                            {
                                                ToastKitManager.ServiceInstaller.Uninstall(service, OnServiceInstallCompleted);
                                            }
                                        });
                                    }
                                    EditorGUI.EndDisabledGroup();
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        for (int i = 0; i < service.linkList.Count; i++)
                        {
                            ...
                            if (ManagerUi.LabelButton(link.name) == true)
                            {
                                ...
                                ToastKitManagerIndicator.SendLink(service.title, link.name, link.url);
                            }
        ...
        private void OnServiceInstallCompleted(ManagerError error)
        {
            ...
            if (ToastKitManagerWindow.window != null)
            {
                ToastKitManagerWindow.window.Repaint();
            }
        }
    }
}