...
namespace Toast.Kit.Manager.Ui.Helper
{
    internal static partial class ManagerUi
    {
        ...
        public static bool ErrorDialog(ManagerError error)
        {
            ...
            string message = ManagerInfos.GetString(error.Message);
            ...
            ToastKitLogger.Error(string.Format("{0} - {1}", error.ErrorCode, message), ManagerInfos.SERVICE_NAME, typeof(ManagerUi));
        ...
        private static string GetValueString(string text)
        {
            if (string.IsNullOrEmpty(text) == false && text.StartsWith(ManagerInfos.SERVICE_INFO_MULTILANGUAGE_SEPARATOR) == true)
            {
                return ToastKitMultilanguage.GetString(ToastKitManager.Instance.ServiceLanguageName, text);
            }
